...
#include "conversion_helper.h"      // for get_serialization_header
#include "deserialization_helper.h" // for deserialization_helper_t, deseri...
#include "sstable_data.h"           // for sstable_data_t, sstable_data_t::...
#include "sstable_statistics.h"     // for sstable_statistics_t::serializat...
...
void debug_statistics(const std::unique_ptr<sstable_statistics_t> &statistics)
{
    ...
    std::cout << "min ttl: " << body->min_ttl()->val() << '\n'
...
void debug_data(const std::unique_ptr<sstable_data_t> &sstable)
{
    std::cout << "========== data file ==========\n";
    for (auto &partition : *sstable->partitions())
    {
        ...
        for (auto &unfiltered : *partition->unfiltereds())
        {
            ...
            if ((unfiltered->flags() & 0x02) != 0) // range tombstone marker
            {
                ...
                // sstable_data_t::range_tombstone_marker_t *marker =
                // (sstable_data_t::range_tombstone_marker_t
                // *)unfiltered->body();
            }
            else
            {
                ...
                for (int i = 0; i < deserialization_helper_t::get_n_cols(deserialization_helper_t::REGULAR); ++i)
                {
                    if (deserialization_helper_t::is_multi_cell(deserialization_helper_t::REGULAR, i))
                    {
                        ...
                        sstable_data_t::complex_cell_t *cell =
                            (sstable_data_t::complex_cell_t *)(*row->cells())[i].get();
                        for (const auto &simple_cell : *cell->simple_cells())
                            std::cout << "child path, value as string: " << simple_cell->path()->value() << " | "
                                      << simple_cell->value() << '\n';
                    }
                    else
                    {
