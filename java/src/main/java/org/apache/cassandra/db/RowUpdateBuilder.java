...
public class RowUpdateBuilder
{
    ...
    {
        this(metadata, ApolloTime.systemClockSecondsAsInt(), timestamp, partitionKey);
    }

    public RowUpdateBuilder(TableMetadata metadata, int localDeletionTime, long timestamp, Object partitionKey)
    {
        this(metadata, localDeletionTime, timestamp, metadata.params.getInt(TableParams.DEFAULT_TTL), partitionKey);
    }

    public RowUpdateBuilder(TableMetadata metadata, long timestamp, int ttl, Object partitionKey)
    {
        this(metadata, ApolloTime.systemClockSecondsAsInt(), timestamp, ttl, partitionKey);
    }
    ...
    {
        return deleteRowAt(metadata, timestamp, ApolloTime.systemClockSecondsAsInt(), key, clusteringValues);
    }
